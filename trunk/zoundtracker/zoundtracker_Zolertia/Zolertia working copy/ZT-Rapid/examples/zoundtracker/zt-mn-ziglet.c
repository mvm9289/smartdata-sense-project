...
// #define NUM_SECONDS_SAMPLE 6  // Default 60 (1 sample/min)
...
{   
    ...
    uint16_t batt = battery_sensor.value(0);
     ...
     packet_to_send.reserved[21] = (unsigned char) (batt & 0x00FF);
	  ...
	  printf("  Data: %d-%d-%d-%d\n", packet_to_send.data[0], packet_to_send.data[1], packet_to_send.data[2], packet_to_send.data[3]);
	  printf("  Checksum: %d\n", packet_to_send.checksum);
          printf("  Battery: %d-%d\n\n", packet_to_send.reserved[20], packet_to_send.reserved[21]);
	#endif
...
get_sensor_sample(void)
{
    ...
    if(write_attempts == 0) {
	 ...
	 printf(" sample value: %d\n\n", valBuff[1]);
        ...
        current_sample.value[0] = (char)(valBuff[1] & 0x00FF);
        current_sample.value[1] = (char)((valBuff[1] & 0xFF00)>>8);
    }

    if (lastRead != firstRead){
...
PROCESS_THREAD(example_zoundt_mote_process, ev, data) {   
    ...
    broadcast_open(&zoundtracker_broadcast_conn, CHANNEL2, &zoundtracker_broadcast_callbacks);       
    ...
    etimer_set(&control_timer, ZT_READ_INTERVAL/3);
    sample_interval = 0;
