                         ...
                         uint8_t hops    )
{
    ...
    #ifdef DEBUG_NET
      printf("[Net]\n  Packet received from [%d.%d] len [%d]\n\n", from->u8[0], 
             from->u8[1], packetbuf_datalen());
    #endif
    ...
    if (packetbuf_datalen() >= PACKET_SIZE)
    {
        ...
        if (recv_checksum != recv_packet.checksum)
        {
                ...
                printf("checksum\n\n");
                printf("             Mesh received callback: Sending POLL ");
                printf("packet\n\n");
            #endif
        ...
        else
        {
            ...
            else if (recv_packet.type == DATA)
            {
                ...
                recv_packet.reserved[3] = (char)(rssi);
                    ...
                    printf("             BATTERY: %d\n\n",
                            (recv_packet.reserved[18]<<8) + 
                             recv_packet.reserved[19]);
                    printf("             DATA:\n             ");
                    for (i = 0; i < DATA_SIZE && 
                         recv_packet.counter + i < recv_packet.size; i++) {
                       printf("(%d) ", recv_packet.data[i]);
                       i++;            
                       temp = (recv_packet.data[i]<<8);
                       i++;  
                       temp += recv_packet.data[i];
                       printf("%u ", temp);
                       if(i== 20) printf("\n             ");
                     }
                    printf("\n");
