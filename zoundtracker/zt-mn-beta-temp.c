...
/* Sensor */
#define TEMP_SAMPLE_SIZE sizeof(TempSample)
...
/* Sensor */
static char minus;
static int sample_interval, tempint, raw, sign;
static unsigned int tempfrac, absraw; 
static TempSample current_sample;
...
prepare_packet(void)
{
	...
	{                  
        read_bytes = cfs_read(fd_read, read_buffer, 2*TEMP_SAMPLE_SIZE);
		#ifdef DEBUG_CFS
	      printf("[cfs]\n Num. bytes readed: %d\n\n", read_bytes);
		#endif
...
{
    /* [Functionality]
     This functions takes a sample of the temperature sensor. 
     Then stores the pair (number of sample, sensor sample) into 
    ...
    current_sample.minus = minus;
    current_sample.tempint = tempint;
    current_sample.tempfrac = tempfrac;
    ...
    {
        write_bytes = cfs_write(fd_write, &current_sample, TEMP_SAMPLE_SIZE);
        if (write_bytes != TEMP_SAMPLE_SIZE) 
		{
			#ifdef DEBUG_CFS
